<%-- 
    Document   : ordersven
    Created on : 28/11/2019, 01:36:38 PM
    Author     : Jeobany
--%>
...
<%
    ...
    ArrayList<String> product=obj.getOrderVen(id, prod);
    ArrayList<ArrayList<String>> prodcom=obj.getProductComments(prod);
    String msj = "";
    int vStat = 0;
    String sStat="";
    if(product.get(16).equalsIgnoreCase("1")){
        vStat = 20;
            sStat= "En Proceso";
    
    }else if(product.get(16).equalsIgnoreCase("2")){
        vStat = 40;
            sStat= "Enviado";
    
    }else if(product.get(16).equalsIgnoreCase("3")){
        vStat = 60;
            sStat= "En Camino";
    
    }else if(product.get(16).equalsIgnoreCase("4")){
        vStat = 80;
            sStat= "En Entrega";
    
    }else if(product.get(16).equalsIgnoreCase("5")){
        vStat = 100;
            sStat= "Entregado";
    ...
    String btn = request.getParameter("done");
        if(btn!=null)
        {
            ...
            String statusX = request.getParameter("estatus");
            if(statusX.equalsIgnoreCase("En proceso")){
                stt = "1";
            }else if(statusX.equalsIgnoreCase("Enviado")){
                stt = "2";
            }else if(statusX.equalsIgnoreCase("En Camino")){
                stt = "3";
            }else if(statusX.equalsIgnoreCase("En entrega")){
                stt = "4";
            }else if(statusX.equalsIgnoreCase("Entregado")){
                stt = "5";
            }
            
            
            msj = obj.updateStatus(id,prod,stt);
            System.out.println("El mensaje " + msj);
            System.out.println("El estatus " + statusX);
           response.sendRedirect("ordersven.jsp?product="+prod);
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    ...
    <div class="row py-md-5">
        ...
        <!--   <p>Precio: <del>$700</del> $650</p> -->
           ...
           <p>Estatus Actual: <%=sStat%></p>
           
           <form method="POST" action="ordersven.jsp">
           <label for="estatus">Cambiar Estatus</label>
                            <select class="form-control" id="estatus" name="estatus">
                                <option name="Proceso" id="proc" <%if(product.get(16).equalsIgnoreCase("1")){%>selected="true"<%}%>>En proceso</option>
                              
                                <option name="Enviado" id="env" <%if(product.get(16).equalsIgnoreCase("2")){%>selected="true"<%}%>>Enviado</option>
                              <option name="Camino" id="cam" <%if(product.get(16).equalsIgnoreCase("3")){%>selected="true"<%}%>>En Camino</option>
                              <option name="Entrega" id="entr" <%if(product.get(16).equalsIgnoreCase("4")){%>selected="true"<%}%>>En entrega</option>
                              <option name="Entrega" id="entr"<%if(product.get(16).equalsIgnoreCase("5")){%>selected="true"<%}%>>Entregado</option>
                              
                            </select>
                              <input type="hidden" id="product" name="product" value="<%=prod%>" />
                              <button class="btn btn-success" id="done" name ="done" onclick="" >Actualizar</button> 
           </form>
